...
protocol ReloadTableDataDelegate {

    func tableDataRefresh()
}

class CustomQuestionsViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate,AudioDelegate
{
    ...
    var index = Int()
    var assets = [PHAsset]()
    var delegate: ReloadTableDataDelegate?
...
//        present(alert, animated: true, completion: nil)
    ...
    @IBAction func ssaveButtonAction(_ sender: Any)
    {
        ...
        let question :String = (questionTextView.text.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines))
         let answer :String =  (self.textAnswerTextView.text.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines))
        if question.isEmpty {
         ProjectManager.sharedInstance.showAlertwithTitle(title:"", desc: "Please enter question", vc: self)
         return
        }
         ...
         //   let todoEndpoint: String = "http://dev.loopleap.com/api/savecustomquestion"
            ...
            if self.audioUrl != nil || self.videoURL != nil || answer != ""
            {


                if self.textImage.image == UIImage(named: "radioFill-btn") && !answer.isEmpty
                {

                    self.parameters = ["new_question":question, "answer":answer, "answer_by":"write", "video-blob": ""]
                }

                if self.audioImage.image == UIImage(named: "radioFill-btn") && self.audioUrl != nil
                {

                    self.parameters = ["new_question":question, "answer":answer, "answer_by":"record"]
                }

                if self.videoImage.image == UIImage(named: "radioFill-btn") && self.videoURL != nil
                {
                ...
                do{
                    ...
                    else if self.videoImage.image == UIImage(named: "radioFill-btn") && self.videoURL != nil
                    {
                    ...
                    if self.audioImage.image == UIImage(named: "radioFill-btn") && self.audioUrl != nil     //asset.mediaType == .audio
                    {
                        ...
                        }, to:baseURL + ApiMethods.savecustomquestion, headers:headers)
                        { (result) in
...
//                            ProjectManager.sharedInstance.stopLoader()
                            ...
                            case .success(let upload, _, _):
                                ...
                                upload.responseJSON { response in
                                    ...
                                    if response.result.isFailure {
                                        DispatchQueue.main.async {                                            ProjectManager.sharedInstance.showServerError(viewController:self)
                                            ProjectManager.sharedInstance.stopLoader()
                    ...
                    else if self.textImage.image == UIImage(named: "radioFill-btn") && !answer.isEmpty                    {
                        
                         ProjectManager.sharedInstance.showLoader()


                        Alamofire.request(baseURL + ApiMethods.savecustomquestion, method: .post,  parameters: self.parameters, encoding: JSONEncoding.default, headers:headers)
