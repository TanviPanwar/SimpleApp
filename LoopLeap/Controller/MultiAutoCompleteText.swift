//
//  MultiAutoCompleteText.swift
//  LoopLeap
//
//  Created by IOS3 on 02/01/19.
...
open class MultiAutoCompleteTextField:UITextField {
    ...
    fileprivate func setupAutocompleteTable(_ view:UIView){
        ...
        let tableView = UITableView(frame: CGRect(x: self.frame.origin.x, y: self.frame.origin.y + self.frame.height + 4 , width: screenSize.width - (self.frame.origin.x * 2), height: 30.0)) //self.frame.origin.y + self.frame.height + self.frame.height  467
